...
public class UnparametrizedComponentInstance {
	...
	public UnparametrizedComponentInstance(final ComponentInstance composition) {
		Map<String, List<ComponentInstance>> resolvedRequiredInterfaces = composition.getSatisfactionOfRequiredInterfaces();
		this.satisfactionOfRequiredInterfaces = new HashMap<>();
		resolvedRequiredInterfaces.keySet().forEach(r -> this.satisfactionOfRequiredInterfaces.put(r, resolvedRequiredInterfaces.get(r).stream().map(ci -> new UnparametrizedComponentInstance(ci)).collect(Collectors.toList())));
