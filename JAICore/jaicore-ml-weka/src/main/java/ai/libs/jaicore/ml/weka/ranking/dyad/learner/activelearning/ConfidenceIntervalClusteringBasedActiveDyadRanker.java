package ai.libs.jaicore.ml.weka.ranking.dyad.learner.activelearning;
...
public class ConfidenceIntervalClusteringBasedActiveDyadRanker extends ARandomlyInitializingDyadRanker {
	...
	public void activelyTrainWithOneInstance() throws InterruptedException {
		...
		PriorityQueue<List<IDyad>> clusterQueue = new PriorityQueue<>(new ListComparator());

		DyadRankingDataset minibatch = new DyadRankingDataset();
		Map<IDyad, SummaryStatistics> dyadStats = this.getDyadStats();
		...
		for (IVector inst : this.getInstanceFeatures()) {
			...
			try {
				...
				List<List<IDyad>> instanceClusters = new ArrayList<>();
	...
	private double getConfidenceIntervalOverlapForDyads(final IDyad dyad1, final IDyad dyad2) {
	...
	private class ListComparator implements Comparator<List<IDyad>> {
		...
		@Override
		public int compare(final List<IDyad> o1, final List<IDyad> o2) {
			if (o1.size() > o2.size()) {
				return -1;
			}
			if (o1.size() < o2.size()) {
