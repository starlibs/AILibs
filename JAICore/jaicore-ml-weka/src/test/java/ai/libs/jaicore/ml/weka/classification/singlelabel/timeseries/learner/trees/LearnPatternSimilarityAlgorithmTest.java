...
public class LearnPatternSimilarityAlgorithmTest {
	...
	public void generateSubseriesFeatureInstanceTest() {
		...
		assertEquals("The number of attributes does not match the expected number of attributes.", this.segments.length + this.segmentsDifference.length, resultInstance.numAttributes());
	...
	public void generateSubseriesFeaturesInstancesTest() {
		...
		// Check number of attributes
		assertEquals("The number of attributes does not match the expected number of attributes.", this.segments.length + this.segmentsDifference.length, resultInstances.numAttributes());
	...
	public void initializeRegressionTreeTest() {
		...
		assertEquals("The random regression tree uses a different seed than configured after initalization.", this.algorithm.getConfig().seed(), regTree.getSeed());
		assertEquals("The random regression tree uses a different maximum tree depth than configured after initalization.", this.algorithm.getConfig().maxDepth(), regTree.getMaxDepth());
		assertEquals("The random regression tree uses a different K value than configured after initalization.", 1, regTree.getKValue());
	}
	...
	public void generateSegmentsAndDifferencesForTreeTest() {
		...
		assertTrue("A start element of the generated segments is outside of the valid range.", Arrays.stream(segments).anyMatch(i -> i > 0 && i < (timeSeriesLength - length)));
		assertTrue("A start element of the generated segment differences is outside of the valid range.", Arrays.stream(segmentsDifference).anyMatch(i -> i > 0 && i < (timeSeriesLength - length - 1)));
	}
}
