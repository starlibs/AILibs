...
import ai.libs.jaicore.ml.classification.singlelabel.timeseries.learner.trees.TimeSeriesBagOfFeaturesClassifier;
import ai.libs.jaicore.ml.classification.singlelabel.timeseries.learner.trees.TimeSeriesBagOfFeaturesLearningAlgorithm;
...
public class TimeSeriesBagOfFeaturesAlgorithmTest {
	...
	@Before
	public void setup() {
		TimeSeriesBagOfFeaturesClassifier classifier = new TimeSeriesBagOfFeaturesClassifier(SEED, NUM_BINS, NUM_FOLDS, Z_PROP, MIN_INTERVAL_LENGTH, USE_Z_NORMALIZATION);
		this.algorithm = new TimeSeriesBagOfFeaturesLearningAlgorithm(classifier.getConfig(), null, null);
	}
	...
	public void discretizeProbsTest() {
		...
		Assert.assertArrayEquals("The calculated discretized probabilites do not match the expected results.", new int[] { 1, 2, 2 }, TimeSeriesBagOfFeaturesLearningAlgorithm.discretizeProbs(NUM_BINS, probs)[0]);
	}
	...
	public void formHistogramsAndRelativeFreqsTest() {
		...
		// Check histograms
		Assert.assertArrayEquals("The calculated histograms at first place do not match the expected results.", new int[] { 0, 1, 0, 0, 0 }, histograms[0][0]);
		Assert.assertArrayEquals("The calculated histograms at second place do not match the expected results.", new int[] { 0, 0, 1, 0, 0 }, histograms[0][1]);

		// Check relative freqs
		Assert.assertArrayEquals("The calculated relative class frequencies do not match the expected results.", new int[] { 0, 0, 1 }, relativeFreqs[0]);
	...
	public void generateHistogramInstancesTest() {
		...
		Assert.assertArrayEquals("The calculated histogram instances do not match the expected results.", new double[] { 1, 0, 1, 0, 1, 1, 0, 2 },
				TimeSeriesBagOfFeaturesLearningAlgorithm.generateHistogramInstances(histograms, relativeFreqsOfClasses)[0], EPS);
	}
	...
	public void generateSubsequencesAndIntervalsTest() {
		...
		final Pair<int[][], int[][][]> subsequencesAndIntervals = this.algorithm.generateSubsequencesAndIntervals(r, d, lMin, T);
		...
		Assert.assertTrue("The range of the first intervals is not greater equals than the interval minimum length.", intervals[0][0][1] - intervals[0][0][0] >= MIN_INTERVAL_LENGTH);
		Assert.assertTrue("The range of the first subsequence is not greater equals than the minimum subsequence length.", subsequences[0][1] - subsequences[0][0] >= lMin);
	}
}
