...
public class ShapeletTransformAlgorithmTest {
	 ...
	 */
	private ShapeletTransformLearningAlgorithm algorithm;
	...
	@BeforeEach
	public void setup() {
		ShapeletTransformTSClassifier classifier = new ShapeletTransformTSClassifier(K, NUM_CLUSTERS, QUALITY_MEASURE, SEED, CLUSTER);
		this.algorithm = classifier.getLearningAlgorithm(null);
		this.sampleInstanceVector = new double[] { 1, 2, 3 };
	}
	...
	public void generateCandidatesTest() {
		...
		Set<Shapelet> actResult = ShapeletTransformLearningAlgorithm.generateCandidates(data, l, 0);
		Assert.assertEquals("The number of generated candidates does not match the expected number of generated candidates.", 4, actResult.size());

		Shapelet expectedShapelet = new Shapelet(TimeSeriesUtil.zNormalize(new double[] { 1, 2, 3 }, true), 0, 3, 0);
		Assert.assertTrue("The generated shapelets do not match the expected normalized shapelets.", actResult.stream().anyMatch(s -> s.equals(expectedShapelet)));
	...
	public void findDistancesTest() {
		...
		List<Double> actResult = this.algorithm.findDistances(shapelet, dataMatrix);

		Assert.assertEquals("A distance has to be found for each instance!", dataMatrix.length, actResult.size());

		Assert.assertEquals("The first generated distance does not match the expected distance.", 0, actResult.get(0), EPS_DELTA);
		Assert.assertEquals("The second generated distance does not match the expected distance.", 2d / 3, actResult.get(1), EPS_DELTA); // (1.224744871 * 1.224744871 * 2) / 3,

		// Using the optimized version
		this.algorithm.setMinDistanceSearchStrategy(new EarlyAbandonMinimumDistanceSearchStrategy(true));
		actResult = this.algorithm.findDistances(shapelet, dataMatrix);
	...
	public void removeSelfSimilarTest() {
		...
		Assert.assertEquals("The number of retained shapelets does not match the expected number.", 4, ShapeletTransformLearningAlgorithm.removeSelfSimilar(shapelets).size());
	}
	...
	public void mergeTest() {
		...
		List<Map.Entry<Shapelet, Double>> actResult = ShapeletTransformLearningAlgorithm.merge(k, shapelets, newShapelets);

		Assert.assertEquals("The number of retained elements does not match the expected number.", k, actResult.size());
		Assert.assertTrue("The retained shapelets do not contain an element which should be contained.", actResult.contains(testEntry1));
		Assert.assertFalse("The retained shapelets contain an element which sould not be contained.", actResult.contains(testEntry2));
	}
	...
	public void zNormalizeTest() {
		...
		double[] actResult = TimeSeriesUtil.zNormalize(this.sampleInstanceVector, true);
		Assert.assertArrayEquals("The normalized values of the vector do not match the expected results.", expectedResult, actResult, EPS_DELTA);
	}

	/**
	 * See
	 * {@link ShapeletTransformLearningAlgorithm#getMinimumDistanceAmongAllSubsequences(Shapelet, double[])}
	 * and
	...
	public void getMinimumDistanceAmongAllSequencesOptimizedTest() {
		...
		Shapelet shapelet = new Shapelet(TimeSeriesUtil.zNormalize(this.sampleInstanceVector, true), 0, 3, 0);

		EarlyAbandonMinimumDistanceSearchStrategy optSearchStrategy = new EarlyAbandonMinimumDistanceSearchStrategy(true);
		...
		Assert.assertEquals("The result of the exhaustive search strategy does not match the expected result.", 0.0, actResult, EPS_DELTA);

		Assert.assertEquals("The result of the optimimum search strategy does not match the expected result.", actResult, optSearchStrategy.findMinimumDistance(shapelet, matrix), EPS_DELTA);

		Shapelet shapelet2 = new Shapelet(TimeSeriesUtil.zNormalize(new double[] { 1, 4, 2 }, true), 0, 3, 0);
		Assert.assertEquals("The result of the exhaustive search strategy does not match the expected result.", 0.024025991917379048d, unoptSearchStrategy.findMinimumDistance(shapelet2, matrix), EPS_DELTA);
	}
	...
	public void squareEuclideanDistanceTest() {
		...
		Assert.assertEquals("The calculation of the single squared euclidean distance does not match the expected result.", 8d, MathUtil.singleSquaredEuclideanDistance(vector, vector2), EPS_DELTA);
	}

	/**
	 * See {@link
	 * ShapeletTransformLearningAlgorithm#getMinimumDistanceAmongAllSubsequences(Shapelet,
	 * double[])} and {@link
	...
	public void getMinimumDistanceAmongAllSequencesOptimizedTest2() {
		...
		double oldOptimizedResult = optSearchStrategy.findMinimumDistance(shapelet, matrix);

		Assert.assertEquals("The minimum distance of the unoptimzied search strategy does not match the result of the optimzed search strategy.", oldResult, oldOptimizedResult, EPS_DELTA);
	}
}
