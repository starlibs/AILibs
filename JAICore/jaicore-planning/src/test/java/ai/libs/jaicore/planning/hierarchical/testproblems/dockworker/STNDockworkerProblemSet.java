package ai.libs.jaicore.planning.hierarchical.testproblems.dockworker;
...
public class STNDockworkerProblemSet extends AAlgorithmTestProblemSet<STNPlanningProblem> {
	...
	public static STNPlanningProblem getDockworkerProblem(final Monom init) {
	...
	public static STNPlanningProblem getDefaultDockworkerProblem() {
		...
		return getDockworkerProblem(init);
	}

	public static STNPlanningProblem getDockworkerProblemForNumberOfContainers(final int a, final int b, final int c) {
		...
		if (b < 0) {
			throw new IllegalArgumentException("Parameter 'b' must be non-negative");
		}
		if (c < 0) {
			throw new IllegalArgumentException("Parameter 'c' must be non-negative");
		}

		/* define init situation, w.r.t. example in Ghallab, Nau, Traverso; p. 230 */
		Monom init = new Monom(""
				+ "attached('p1a','l1') & attached('p1b','l1') & attached('p1c','l1') & belong('crane1','l1') & empty('crane1') & in('c11','p1a') & in('c12','p1a') & top('c11','p1a') & top('pallet','p1b') & top('pallet','p1c') &"
				+ "attached('p2a','l2') & attached('p2b','l2') & attached('p2c','l2') & belong('crane2','l2') & empty('crane2') & in('c21','p2a') & in('c22','p2a') & in('c23','p2a')  & top('c21','p2a') & top('pallet','p2b') & top('pallet','p2c') &"
				+ "attached('p3a','l3') & attached('p3b','l3') & attached('p3c','l3') & belong('crane3','l3') & empty('crane3') & in('c31','p3a') & in('c32','p3a') & in('c33','p3a') & in('c34','p3a') & top('c31','p3a') & top('pallet','p3b') & top('pallet','p3c')");

		/* define first stack */
		if (a > 0) {
			init.add(new Literal("on('c11','pallet')"));
			for (int i = 2; i <= a; i++) {
				init.add(new Literal("on('c1" + i + "', 'c1" + (i - 1) + "')"));
			}
		}

		/* define second stack */
		if (b > 0) {
			init.add(new Literal("on('c21','pallet')"));
			for (int i = 2; i <= b; i++) {
				init.add(new Literal("on('c2" + i + "', 'c2" + (i - 1) + "')"));
			}
		}

		/* define third stack */
		if (c > 0) {
			init.add(new Literal("on('c31','pallet')"));
			for (int i = 2; i <= c; i++) {
				init.add(new Literal("on('c3" + i + "', 'c3" + (i - 1) + "')"));
			}
		}
		return getDockworkerProblem(init);
	}
	...
	public STNPlanningProblem getSimpleProblemInputForGeneralTestPurposes() {
		return getDefaultDockworkerProblem();
	}
