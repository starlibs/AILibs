...
public class CEOCTFDPathUnifier implements IPathUnification<TFDNode> {
	...
	private static Logger logger = LoggerFactory.getLogger(CEOCTFDGraphGenerator.class);
	...
	public List<TFDNode> getSubsumingKnownPathCompletion(Map<List<TFDNode>, List<TFDNode>> knownPathCompletions, List<TFDNode> path) throws InterruptedException {
		...
		for (List<TFDNode> partialPath : knownPathCompletions.keySet()) {
			List<Long> partialPathIds = partialPath.stream().map(n -> n.getID()).collect(Collectors.toList());
			List<TFDNode> compl = knownPathCompletions.get(partialPath);
