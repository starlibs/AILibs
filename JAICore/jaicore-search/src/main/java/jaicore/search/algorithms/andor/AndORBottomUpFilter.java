...
public class AndORBottomUpFilter<N, A, V extends Comparable<V>> extends AAlgorithm<GraphGenerator<N, A>, Graph<N>> implements IGraphAlgorithm<GraphGenerator<N, A>, Graph<N>, N, A> {
	...
	public AlgorithmEvent nextWithException() throws TimeoutException, InterruptedException, AlgorithmException, AlgorithmExecutionCanceledException {
		...
		case created: {
			...
			root.val = 0;
			open.add(root);
			this.post(new GraphInitializedEvent<>(root.node));
	...
	private Queue<EvaluatedGraph> filterNodeSolution(final InnerNodeLabel node) throws TimeoutException, InterruptedException, ObjectEvaluationFailedException, AlgorithmExecutionCanceledException {
		...
		if (node.type == NodeType.AND) {
			...
			LDSRelationComputer<EvaluatedGraph> cartesianProductBuilder = new LDSRelationComputer<>(new RelationComputationProblem<>(subSolutionsPerChild, subSolutionCombination -> {
				...
				if (!validSolution) {
					logger.debug("\tCutting of sub-solution combination at level " + subSolutionCombination.size() + "/" + subSolutionsPerChild.size() + " as cost function returned " + extendedSolutionBase.value
							+ ". The following combined solution graph was subject of evaluation (one path per line, ommitted nodes are equal to the above line(s)):\n" + extendedSolutionBase.graph.getLineBasedStringRepresentation(2));
				}
