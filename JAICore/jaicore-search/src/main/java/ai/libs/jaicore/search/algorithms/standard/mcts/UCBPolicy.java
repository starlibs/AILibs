...
public class UCBPolicy<T,A> implements IPathUpdatablePolicy<T,A,Double>, ILoggingCustomizable {
	...
	@Override
	public void updatePath(final List<T> path, final Double score) {
	...
	public A getAction(final T node, final Map<A,T> actionsWithTheirSuccessors) {
		...
		for (A action : possibleActions) {
			...
			this.logger.trace("Considering action {} whose successor state has stats {} and {} visits", action, label.scores.getMean(), label.visits);
			double ucb = label.scores.getMean() + (this.maximize ? 1 : -1) * Math.sqrt(2 * Math.log(n) / label.visits);
			...
			if (this.maximize && (ucb > best) || !this.maximize && (ucb < best)) {
				...
				this.logger.trace("Skipping current solution {} since its score {} is not better than the currently best {}.", action, ucb, best);
			}
		}
