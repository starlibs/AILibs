...
public class DNGMCTSFactory<N, A> extends MCTSBuilder<N, A, DNGMCTSFactory<N, A>> {
	...
	public MCTS<N, A> getAlgorithm(final IMDP<N, A, Double> input) {
		return new DNGMCTS<>(input, this.varianceFactor, this.initLambda, this.getMaxIterations(), this.getGamma(), this.getEpsilon(), this.getRandom(), this.isTabooExhaustedNodes());
	}
}
