...
/**
 * This class provides a search graph generator for an augmented version of the
 * timed TSP (TTSP) problem. In TTSP, the original route graph has not only one
 * cost per edge (u,v) but a list of costs where each position in the list
 * represents an hour of departure from u. That is, depending on the time when
 * leaving u, the cost to reach v may vary.
 * 
 * The intended time measure is hours. That is, the length of the lists of all
 * edges should be identical and reflect the number of hours considered.
 * Typically, this is 24 or a multiple of that, e.g. 7 * 24.
 * 
 * Time is considered cyclic. That is, if you consider 24 hours and the current
 * time is 25.5, then it is automatically set to 1.5.
 * 
 * The TTSP is enhanced by the consideration of blocking times for the streets,
 * e.g. on Sundays (as in Germany) or during nights (as in Austria or
 * Switzerland). Blocking times are stored in a separated list of Booleans.
 * 
 * Also, the TTSP is enhanced in that driving pauses need to be considered. That
 * is, the driver must only drive x hours in a row and then make a break of
 * duration y. In addition, there must be an uninterrupted break of z hours
 * every day.
 * 
 * IT IS ASSUMED THAT BREAKS CAN ONLY BE MADE AT LOCATIONS AND NOT "ON THE
 * ROAD"!
 * 
 * @author fmohr
 *
...
public class EnhancedTTSP {
	...
	public EnhancedTTSP(LabeledGraph<Short, Double> minTravelTimesGraph, short startLocation,
			List<Boolean> blockedHours, double hourOfDeparture, double maxConsecutiveDrivingTime,
	...
	public SerializableGraphGenerator<EnhancedTTSPNode, String> getGraphGenerator() {
		return new SerializableGraphGenerator<EnhancedTTSP.EnhancedTTSPNode, String>() {
			...
			public SuccessorGenerator<EnhancedTTSPNode, String> getSuccessorGenerator() {
				return new SingleSuccessorGenerator<EnhancedTTSP.EnhancedTTSPNode, String>() {
					
					private List<Short> getPossibleDestinations(EnhancedTTSPNode n) {
						List<Short> possibleDestinations = SetUtil.intersection(n.unvisitedLocations,
								minTravelTimesGraph.getSuccessors(n.curLocation)).stream().sorted().collect(Collectors.toList());
					...
					@Override
					public List<NodeExpansionDescription<EnhancedTTSPNode, String>> generateSuccessors(
							EnhancedTTSPNode node) {
					...
					public NodeExpansionDescription<EnhancedTTSPNode, String> generateSuccessor(EnhancedTTSPNode n, List<Short> destinations,
							int i) {
							 ...
							 */
							short curLocation = n.curLocation;
							int N = destinations.size();
							short destination = destinations.get(i % N);
							
								double minTravelTime = minTravelTimesGraph.getEdgeLabel(curLocation, destination);
								logger.info("Simulating the ride from " + n.curLocation + " to " + destination + ", which minimally takes " + minTravelTime + ". We are departing at " + n.time);

								double timeToNextShortBreak = getTimeToNextShortBreak(n.time, Math.min(n.timeTraveledSinceLastShortBreak, n.timeTraveledSinceLastLongBreak));
								double timeToNextLongBreak = getTimeToNextLongBreak(n.time, n.timeTraveledSinceLastLongBreak);
								...
								double curTime = n.time;
								boolean arrived = false;
								double shareOfTheTripDone = 0.0;
								double timeOfArrival = -1;
								double timeSinceLastShortBreak = n.timeTraveledSinceLastShortBreak;
								double timeSinceLastLongBreak = n.timeTraveledSinceLastLongBreak;
								while (!arrived) {
									double permittedTimeToTravel = Math.min(timeToNextShortBreak, timeToNextLongBreak);
									double travelTimeWithoutBreak = getActualDrivingTimeWithoutBreak(minTravelTime, curTime,
											shareOfTheTripDone);
									...
									if (permittedTimeToTravel >= travelTimeWithoutBreak) {
										...
										logger.info("\tDriving the remaining distance to goal without a break. This takes " + travelTimeWithoutBreak + " (min time for this distance is " + minTravelTime * (1-shareOfTheTripDone) + ")");
									} else {
										...
										boolean doLongBreak = permittedTimeToTravel == timeToNextLongBreak
												|| permittedTimeToTravel + durationOfShortBreak + 2 >= timeToNextLongBreak;
										if (doLongBreak) {
								...
								double travelDuration = curTime - n.time;
								logger.info("Finished travel simulation. Travel duration: " + travelDuration);
								
								Set<Short> unvisitedLocations = new HashSet<>(n.unvisitedLocations);
								unvisitedLocations.remove(destination);
								assert unvisitedLocations.size() < n.unvisitedLocations
										.size() : "The unvisited cities haven't been reduced!";
								EnhancedTTSPNode newNode = new EnhancedTTSPNode(destination, unvisitedLocations, timeOfArrival,
										timeSinceLastShortBreak, timeSinceLastLongBreak);
								return new NodeExpansionDescription<EnhancedTTSP.EnhancedTTSPNode, String>(n, newNode,
									n.getCurLocation() + " -> " + destination, NodeType.OR);
					}

					@Override
					public NodeExpansionDescription<EnhancedTTSPNode, String> generateSuccessor(EnhancedTTSPNode node,
							int i) {
			...
			public double getActualDrivingTimeWithoutBreak(double minTravelTime, double departure,
			...
			public double getShareOfTripWhenDrivingOverEdgeAtAGivenTimeForAGivenTimeWithoutDoingABreak(
					double minTravelTime, double departureOfCurrentPoint, double shareOfTripDone, double drivingTime) {
			...
			public NodeGoalTester<EnhancedTTSPNode> getGoalTester() {
				return n -> {
					return n.getUnvisitedLocations().isEmpty() && n.curLocation == startLocation;
				};
			}
	...
	public ISolutionEvaluator<EnhancedTTSPNode, Double> getSolutionEvaluator() {
		return new ISolutionEvaluator<EnhancedTTSPNode, Double>() {
			...
			public boolean doesLastActionAffectScoreOfAnySubsequentSolution(
					List<EnhancedTTSPNode> partialSolutionPath) {
				return true;
			}
		};
	}
