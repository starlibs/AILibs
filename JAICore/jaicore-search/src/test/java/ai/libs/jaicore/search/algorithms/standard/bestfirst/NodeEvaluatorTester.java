...
public abstract class NodeEvaluatorTester<N extends IPathEvaluator<EnhancedTTSPState, String, Double>> {
	...
	public StandardBestFirst<EnhancedTTSPState, String, Double> getBF(final int problemSize, final IPathEvaluator<EnhancedTTSPState, String, Double> ne) {
		EnhancedTTSPGenerator generator = new EnhancedTTSPGenerator(new RandomLocationGenerator(new Random(0)));
		EnhancedTTSP problem = generator.generate(problemSize, 100, 1);
		GraphSearchWithSubpathEvaluationsInput<EnhancedTTSPState, String, Double> input = new GraphSearchWithSubpathEvaluationsInput<>(new EnhancedTTSPSimpleGraphGenerator(problem), new EnhancedTTSPSolutionPredicate(problem), ne); // there will
		// be 10
		// solutions
