 ...
 * @param <T>
 */
public class ExperimentRunner<T> extends Thread {

    private List<T> bestSolution = null;
    private double costOfBestSolution = 0d;

    private ORGraphSearch<T, String, Double> search;
    private ISolutionEvaluator<T, Double> solutionEvaluator;

    private boolean noNextSolution = false;

    public ExperimentRunner(ORGraphSearch<T, String, Double> search, ISolutionEvaluator<T, Double> solutionEvaluator) {
        this.search = search;
