...
public class LDSNQueenTester {
	...
	public void testSequential() throws InterruptedException {
		...
		for (int i = 0; i < numbersOfSolutions.length; i++) {
			...
			/* initialize LDS */
			BestFirstLimitedDiscrepancySearch<QueenNode, String> lds = new BestFirstLimitedDiscrepancySearch<>(new NQueenGenerator(n), (n1, n2) -> {
				List<Integer> p1 = n1.getPositions();
				List<Integer> p2 = n2.getPositions();
				if (p1.size() != p2.size())
					throw new IllegalArgumentException("Cannot compare nodes with different numbers of queens!");
