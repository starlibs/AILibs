...
public class EnhancedTTSPTest {
	...
	public static void createProblem() {
		...
		List<Boolean> blockedHours = Arrays.asList(
				new Boolean[] { true, true, true, true, true, true, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, true, true }); // driving
																																																	// blocked
																																																	// between
																																																	// 11pm
																																																	// and
	...
	private void runAlgorithm(String name, IObservableORGraphSearch<EnhancedTTSPNode, String, Double> search, boolean stopOnFirst) {
		...
		System.out.println("Best tour is: " + answer.getX().stream().map(m -> m.getCurLocation()).collect(Collectors.toList()));
		System.out.println("Time of best tour is: " + answer.getY());
	}

	private Pair<List<EnhancedTTSPNode>, Double> runSearch(IObservableORGraphSearch<EnhancedTTSPNode, String, Double> search, boolean stopOnFirst, boolean visualize) {

		/* activate visualizer if desired */
		if (visualize) {
			SimpleGraphVisualizationWindow<Node<EnhancedTTSPNode, Double>> win = new SimpleGraphVisualizationWindow<>(search);
			win.getPanel().setTooltipGenerator(n -> "<table>" + "<tr><td>f-value</td><td>" + n.getInternalLabel() + "</td></tr>" + "<tr><td>f-time</td><td>" + n.getAnnotations() + "</td></tr>"
					+ "<tr><td>Loc</td><td>" + n.getPoint().getCurLocation() + "</td></tr>" + "<tr><td>Unvisited Locations</td><td>" + n.getPoint().getUnvisitedLocations() + "</td></tr>"
					+ "<tr><td>Time</td><td>" + n.getPoint().getTime() + "</td></tr>" + "<tr><td>Time Traveled since last short break</td><td>" + n.getPoint().getTimeTraveledSinceLastShortBreak()
					+ "</td></tr>" + "<tr><td>Time Traveled since last long break</td><td>" + n.getPoint().getTimeTraveledSinceLastLongBreak() + "</td></tr>" + "</table>");
		}
