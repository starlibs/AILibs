...
public class ForwardChainer extends AAlgorithm<ForwardChainingProblem, Collection<Map<VariableParam, LiteralParam>>> {
	...
	public AlgorithmEvent nextWithException() throws InterruptedException, AlgorithmExecutionCanceledException, TimeoutException {
		...
		case created: {
			...
			logger.debug("Selected literal {} with still unbound params {} that can be ground in {} ways in {}ms.", chosenLiteral, chosenLiteral.getVariableParams(), possibleChoicesForLocalLiteral.size(), end - timeToPrepareCWAVersion);
