...
/**
 * This algorithm answers the question for which groundings G of the variables in
 * $premise, G[$premise] follows from $factbase. This method does NOT adopt the
 * closed world assumption, i.e. !L in the premise can only be followed if L is
 * provably wrong in the factbase.
 * 
 * @param factbase
 * @param conclusion
 * @return
...
//	private Collection<Map<VariableParam, LiteralParam>> mappings = new HashSet<>();
	...
	public AlgorithmEvent nextWithException() {
		...
		case created: {
			...
			for (Literal nextLitealCandidate : conclusion) {
				...
				List<Map<VariableParam, LiteralParam>> choicesTmp = getGroundingsUnderWhichALiteralAppearsInFactBase(factbase, nextLitealCandidate, currentlyFewestOptions);
		...
		case active: {
			...
			if (currentlyActiveSubFC != null) {
				...
				if (event == null) {
					...
					assert verifyThatGroundingEnablesConclusion(factbase, currentGroundRemainingConclusion, solutionToReturn);
	//				logger.debug("Finished consistency check for choice {} in {}ms.", currentGroundingOfLocalLiteral, System.currentTimeMillis() );
		//			logger.debug("Done. Recursive consistency check of {} choices for the {}-premise took {}ms and returned {} mappings.", possibleChoicesForLocalLiteral.size(), remainingConclusion.size(), System.currentTimeMillis() - start,
		//					mappings.size());
			...
			logger.debug("Determine a new out of {} remaining groundings for {} to be analyzed.", possibleChoicesForLocalLiteral.size(), chosenLiteral);
			...
			logger.debug("Selected grounding {}. {} possible other groundings remain.", currentGroundingOfLocalLiteral, possibleChoicesForLocalLiteral.size());
			...
			if (currentGroundRemainingConclusion.isEmpty()) {
				...
				ForwardChainingProblem subProblem = new ForwardChainingProblem(factbase, currentGroundRemainingConclusion, getInput().isCwa());
				long startRecursiveCall = System.currentTimeMillis();
	//			Collection<Map<VariableParam, LiteralParam>> subsolutions = getSubstitutionsThatEnableForwardChaining(factbase, currentGroundRemainingConclusion);
				logger.debug("Finished recursion of {}-conclusion. Computation took {}ms", currentGroundRemainingConclusion.size(), System.currentTimeMillis() - startRecursiveCall);
	 ...
	 * @param factbase
	 * @param conclusion
	 * @return true iff the conclusion contains a positive ground literal that is NOT in the factbase or a negative ground literal that positively occurs in the factbase
	 */
