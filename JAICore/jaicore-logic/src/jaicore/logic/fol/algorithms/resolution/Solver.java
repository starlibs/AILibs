...
public abstract class Solver {
	...
	public Map<CNFFormula, CNFFormula> getInternalRepresentationOfFormulas() {
		return internalRepresentationOfFormulas;
	}
	...
	public void addFormula(CNFFormula formula) {
		...
		for (Clause c : formula) {
			...
			for (VariableParam p : paramsInC) {
				substitution.put(p, new VariableParam("c" + varCounter + "_" + p.getName()));
			}
			disAmbFormula.add(new Clause(c, substitution));
			varCounter++;
		}
		internalRepresentationOfFormulas.put(formula, disAmbFormula);
	}

	public void removeFormula(CNFFormula formula) {
		formulas.remove(formula);
		internalRepresentationOfFormulas.remove(formula);
	}

	public boolean isSatisfiable(CNFFormula formula) throws InterruptedException {
		CNFFormula internalRepresentation = internalRepresentationOfFormulas.get(formula);
		CNFFormula resultOfResolution = performResolutionUntilEmptyClauseIsFound(internalRepresentation, internalRepresentation);
		internalRepresentationOfFormulas.put(formula, resultOfResolution);
	...
	public boolean isSatisfiable(CNFFormula formula, CNFFormula formulaToChooseAtLeastOneLiteralFrom) throws InterruptedException {
		CNFFormula internalRepresentationOfFormula1 = internalRepresentationOfFormulas.get(formula);
		CNFFormula internalRepresentationOfFormula2 = internalRepresentationOfFormulas.get(formulaToChooseAtLeastOneLiteralFrom);
		...
		CNFFormula resultOfResolution = performResolutionUntilEmptyClauseIsFound(joint, internalRepresentationOfFormula2);
		return !resultOfResolution.contains(new HashSet<>());
	}
	...
	public boolean isSatisfiable() throws InterruptedException {
		...
		for (CNFFormula formula : formulas) {
			jointFormula.addAll(internalRepresentationOfFormulas.get(formula));
		}
		CNFFormula resultOfResolution = performResolutionUntilEmptyClauseIsFound(jointFormula, jointFormula);
		return !resultOfResolution.contains(new HashSet<>());
	}
	...
	protected CNFFormula performResolutionUntilEmptyClauseIsFound(CNFFormula formula, CNFFormula formulaToChooseAtLeastOneLiteralFrom) throws InterruptedException {
		...
		candidates.addAll(getPossibleResolutionPairs(formulaToChooseAtLeastOneLiteralFrom));
		for (Clause c : formula) {
			candidates.addAll(getPossibleResolutionPairs(formulaToChooseAtLeastOneLiteralFrom, c));
		}

		/* create memory */
		tree = new ResolutionTree(formula);
		...
		while ((nextPair = getNextPair(candidates)) != null) {
			ResolutionStep step = performResolutionStepForPair(nextPair);
			if (step == null)
				continue;
			...
			if (resolvent.isEmpty()) { // cancel if we deduced the empty clause
				...
				tree.addResolutionStep(step);
				// tree.printAsGraphViz("resolution.txt");
				return currentFormula;
			}

			if (isClauseAlreadyContainedInFormula(currentFormula, resolvent)) {
				logger.debug("Ignoring resolvent, because it is already contained in the known formula.");
				continue;
			}

			tree.addResolutionStep(step); // memorize this step
			 ...
			 */
			List<ResolutionPair> successors = getPossibleResolutionPairs(currentFormula, resolvent);
			candidates.addAll(getAdmissiblePairs(successors));
		}
		// tree.printAsGraphViz("resolution.txt");
		return currentFormula;
	}
	...
	protected List<ResolutionPair> getPossibleResolutionPairs(CNFFormula formula) throws InterruptedException {
		...
		for (List<Clause> pair : candidates) {
			...
			pairs.addAll(getPossibleResolutionsPairsOfClauses(invert ? c2 : c1, invert ? c1 : c2));
		}
		return pairs;
	}

	abstract protected List<ResolutionPair> getAdmissiblePairs(List<ResolutionPair> pairs);
	...
	protected boolean isClauseAlreadyContainedInFormula(CNFFormula formula, Clause c) throws InterruptedException {
		if (formula.contains(c))
			return true;
		Set<Clause> possibleMatchings = getClausesWithSamePredicates(formula, c);
	...
	protected ResolutionStep performResolutionStepForPair(ResolutionPair pair) {
		...
		Map<VariableParam, LiteralParam> unifier = getUnificatorForLiterals(pair.getL1(), pair.getL2());
		if (unifier == null)
			return null;
