package ai.libs.jaicore.ml.ranking.dyadranking.algorithm;
 ...
 *
 */
public class PLNetDyadRanker extends ASupervisedLearner<IPLNetDyadRankerConfiguration, Dyad, IRanking<Dyad>, IDyadRankingInstance, DyadRankingDataset>
		implements IPLDyadRanker<IPLNetDyadRankerConfiguration>, IProbabilisticPredictor<Dyad, IRanking<Dyad>, IDyadRankingInstance, DyadRankingDataset> {
	...
	private INDArray computeScaledGradient(final IDyadRankingInstance instance) {
		...
		INDArray dyadMatrix;
		List<INDArray> dyadList = new ArrayList<>(instance.getNumFeatures());
		...
		Gradient deltaWk = null;
		MultiLayerNetwork plNetClone = this.plNet.clone();
		for (int k = 0; k < instance.getNumFeatures(); k++) {
			// compute derivative of loss w.r.t. k
	...
	public void update(final IDyadRankingInstance instance) throws TrainingException {
		if (this.plNet == null) {
			int dyadSize = (instance.get(0).getInstance().length()) + (instance.get(0).getAlternative().length());
	...
	private INDArray dyadRankingToMatrix(final IDyadRankingInstance drInstance) {
		List<INDArray> dyadList = new ArrayList<>(drInstance.getNumFeatures());
	...
	public IDyadRankingInstance getPairWithLeastCertainty(final IDyadRankingInstance drInstance) {
		...
		if (this.plNet == null) {
			int dyadSize = (drInstance.get(0).getInstance().length()) + (drInstance.get(0).getAlternative().length());
			this.plNet = this.createNetwork(dyadSize);
			this.plNet.init();
		}

		if (drInstance.getNumFeatures() < 2) {
			throw new IllegalArgumentException("The query instance must contain at least 2 dyads!");
		}
		List<Pair<Dyad, Double>> dyadUtilityPairs = new ArrayList<>(drInstance.getNumFeatures());
		for (Dyad dyad : drInstance) {
	...
	public double getProbabilityOfTopRanking(final IDyadRankingInstance drInstance) {
		return this.getProbabilityOfTopKRanking(drInstance, drInstance.getNumFeatures());
	}
	...
	@Override
	public void fit(final DyadRankingDataset dTrain) throws TrainingException, InterruptedException {
		this.fit(dTrain.toND4j());
	}

	@Override
	public IPrediction<IRanking<Dyad>> predict(final IDyadRankingInstance xTest) throws PredictionException, InterruptedException {
		if (this.plNet == null) {
			int dyadSize = (xTest.get(0).getInstance().length()) + (xTest.get(0).getAlternative().length());
			this.plNet = this.createNetwork(dyadSize);
			this.plNet.init();
		}

		List<Pair<Dyad, Double>> dyadUtilityPairs = new ArrayList<>(xTest.getNumFeatures());
		for (Dyad dyad : xTest) {
			INDArray plNetInput = this.dyadToVector(dyad);
			double plNetOutput = this.plNet.output(plNetInput).getDouble(0);
			dyadUtilityPairs.add(new Pair<Dyad, Double>(dyad, plNetOutput));
	...
	@Override
	public double getCertainty(final IDyadRankingInstance queryInstance, final IRanking<Dyad> sizeTwoRanking) {
		if (queryInstance.getNumFeatures() != 2) {
			throw new IllegalArgumentException("Can only provide certainty for pairs of dyads!");
		}
		List<Pair<Dyad, Double>> dyadUtilityPairs = new ArrayList<>(sizeTwoRanking.size());
