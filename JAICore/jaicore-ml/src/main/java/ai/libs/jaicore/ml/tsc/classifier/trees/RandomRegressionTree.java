 ...
 */
public class RandomRegressionTree extends AbstractClassifier
implements OptionHandler, WeightedInstancesHandler, Randomizable, Drawable, PartitionGenerator {
	...
	public void buildClassifier(Instances data) throws Exception {
		...
		if (data.classAttribute().isNumeric()) {
			trainVariance = RandomRegressionTree.singleVariance(classProbs[0], totalSumSquared, totalWeight)
					/ totalWeight;
	...
	public String toString() {
		...
		if (this.m_Tree == null) {
			...
			return "\nRandomTree\n==========\n" + this.m_Tree.toString(0) + "\n" + "\nSize of the tree : "
					+ this.m_Tree.numNodes() + (this.getMaxDepth() > 0 ? ("\nMax depth of tree: " + this.getMaxDepth()) : (""));
		}
	}
	...
	protected class Tree implements Serializable {
		...
		public double[] distributionForInstance(final Instance instance) throws Exception {
			...
			if(this.m_Attribute > -1) {
				...
				} else if (RandomRegressionTree.this.m_Info.attribute(this.m_Attribute).isNominal()) {

					// For nominal attributes
					returnedDist = this.m_Successors[(int) instance.value(this.m_Attribute)]
		...
		public int toGraph(final StringBuffer text, int num) throws Exception {
			...
			String classValue = RandomRegressionTree.this.m_Info.classAttribute().isNominal() ? RandomRegressionTree.this.m_Info
					.classAttribute().value(maxIndex) : Utils.doubleToString(
							this.m_ClassDistribution[0], 2);

					num++;
					if (this.m_Attribute == -1) {
						text.append("N" + Integer.toHexString(this.hashCode()) + " [label=\"" + num
								+ ": " + classValue + "\"" + "shape=box]\n");
					} else {
						text.append("N" + Integer.toHexString(this.hashCode()) + " [label=\"" + num
								+ ": " + classValue + "\"]\n");
						for (int i = 0; i < this.m_Successors.length; i++) {
							text.append("N" + Integer.toHexString(this.hashCode()) + "->" + "N"
									+ Integer.toHexString(this.m_Successors[i].hashCode()) + " [label=\""
									+ RandomRegressionTree.this.m_Info.attribute(this.m_Attribute).name());
		...
		protected String leafString() throws Exception {
			...
			if (RandomRegressionTree.this.m_Info.classAttribute().isNumeric()) {
				return " : " + Utils.doubleToString(classMean, 2) + " ("
						+ Utils.doubleToString(this.m_Distribution[1], 2) + "/"
						+ Utils.doubleToString(avgError, 2) + ")";
			}

			return " : " + RandomRegressionTree.this.m_Info.classAttribute().value(maxIndex) + " ("
			...
			try {
				...
				if (this.m_Attribute == -1) {
					...
					for (int i = 0; i < this.m_Successors.length; i++) {
						...
						text.append(RandomRegressionTree.this.m_Info.attribute(this.m_Attribute).name() + " = "
								+ RandomRegressionTree.this.m_Info.attribute(this.m_Attribute).value(i));
					...
					text.append(RandomRegressionTree.this.m_Info.attribute(this.m_Attribute).name() + " < "
							+ Utils.doubleToString(this.m_SplitPoint, 2));
		 ...
		 * @throws Exception if generation fails
		 */
		protected void backfitData(final Instances data, final double[] classProbs,
				final double totalWeight) throws Exception {
				...
				for (int i = 0; i < data.numInstances(); i++) {
					...
					if (!inst.isMissing(this.m_Attribute)) {
						if (data.attribute(this.m_Attribute).isNominal()) {
							...
							this.m_Prop[(inst.value(this.m_Attribute) < this.m_SplitPoint) ? 0 : 1] += inst
									.weight();
						}
					}
				}
		 ...
		 * @throws Exception if generation fails
		 */
		protected void buildTree(final Instances data, final double[] classProbs,
				final int[] attIndicesWindow, double totalWeight, final Random random, final int depth,
			...
			if (totalWeight < 2 * RandomRegressionTree.this.m_MinNum ||
					...
					// check tree depth
					((RandomRegressionTree.this.getMaxDepth() > 0) && (depth >= RandomRegressionTree.this.getMaxDepth()))) {
			...
			while ((windowSize > 0) && (k-- > 0 || !gainFound)) {
				...
				double currSplit = data.classAttribute().isNominal() ? this.distribution(
						props, dists, attIndex, data) : this.numericDistribution(props, dists,
								attIndex, totalSubsetWeights, data, tempNumericVals);

						double currVal = data.classAttribute().isNominal() ? this.gain(dists[0],
								this.priorVal(dists[0])) : tempNumericVals[attIndex];

						if (Utils.gr(currVal, 0)) {
							gainFound = true;
						}

						if ((currVal > val) || ((!RandomRegressionTree.this.getBreakTiesRandomly()) && (currVal == val) && (attIndex < bestIndex))) {
							val = currVal;
							bestIndex = attIndex;
							split = currSplit;
							bestProps = props[0];
		 ...
		 */
		protected double numericDistribution(final double[][] props, final double[][][] dists,
				final int att, final double[][] subsetWeights, final Instances data, final double[] vals)
						throws Exception {
		 ...
		 * @throws Exception if something goes wrong
		 */
		protected double distribution(final double[][] props, final double[][][] dists,
				final int att, final Instances data) throws Exception {
		 ...
		 * @throws Exception if something goes wrong
		 */
		protected int toGraph(final StringBuffer text, int num, final Tree parent)
				throws Exception {

			num++;
			if (this.m_Attribute == -1) {
				text.append("N" + Integer.toHexString(Tree.this.hashCode())
				+ " [label=\"" + num + Utils.backQuoteChars(this.leafString()) + "\""
				+ " shape=box]\n");

			} else {
				text.append("N" + Integer.toHexString(Tree.this.hashCode())
				+ " [label=\"" + num + ": "
				+ Utils.backQuoteChars(RandomRegressionTree.this.m_Info.attribute(this.m_Attribute).name())
				+ "\"]\n");
				for (int i = 0; i < this.m_Successors.length; i++) {
					text.append("N" + Integer.toHexString(Tree.this.hashCode()) + "->"
							+ "N" + Integer.toHexString(this.m_Successors[i].hashCode())
							+ " [label=\"");
					if (RandomRegressionTree.this.m_Info.attribute(this.m_Attribute).isNumeric()) {
						if (i == 0) {
							text.append(" < " + Utils.doubleToString(this.m_SplitPoint, 2));
						...
						text.append(" = "
								+ Utils.backQuoteChars(RandomRegressionTree.this.m_Info.attribute(this.m_Attribute).value(i)));
					}
					text.append("\"]\n");
