package ai.libs.jaicore.ml.ranking.dyadranking.util;
...
public abstract class AbstractDyadScaler implements Serializable {
	...
	public void fit(final DyadRankingDataset dataset) {
		int lengthX = dataset.get(0).get(0).getInstance().length();
		int lengthY = dataset.get(0).get(0).getAlternative().length();
	...
	public void transformInstances(final SparseDyadRankingInstance drInstance, final List<Integer> ignoredIndices) {
		this.transformInstaceVector(drInstance.get(0).getInstance(), ignoredIndices);
	}
