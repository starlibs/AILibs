package ai.libs.jaicore.ml.ranking.dyadranking.algorithm.featuretransform;
...
import ai.libs.jaicore.ml.ranking.dyadranking.optimizing.BilinFunction;
import ai.libs.jaicore.ml.ranking.dyadranking.optimizing.DyadRankingFeatureTransformNegativeLogLikelihood;
import ai.libs.jaicore.ml.ranking.dyadranking.optimizing.DyadRankingFeatureTransformNegativeLogLikelihoodDerivative;
import ai.libs.jaicore.ml.ranking.dyadranking.optimizing.IDyadRankingFeatureTransformPLGradientDescendableFunction;
import ai.libs.jaicore.ml.ranking.dyadranking.optimizing.IDyadRankingFeatureTransformPLGradientFunction;
 ...
 *
 */
public class FeatureTransformPLDyadRanker extends ASupervisedLearner<ILearnerConfig, Dyad, IRanking<Dyad>, IDyadRankingInstance, DyadRankingDataset> implements IPLDyadRanker<ILearnerConfig> {
	...
	@Override
	public IPrediction<IRanking<Dyad>> predict(final IDyadRankingInstance instance) throws PredictionException {
		if (this.w == null) {
		...
		return new Prediction<>(new Ranking<Dyad>(skillForDyads.stream().sorted((p1, p2) -> Double.compare(p1.getX(), p2.getX())).map(Pair<Double, Dyad>::getY).collect(Collectors.toList())));
	}
	...
	@Override
	public void fit(final DyadRankingDataset dataset) throws TrainingException, InterruptedException {
		...
		int alternativeLength = dataset.get(0).get(0).getAlternative().length();
		int instanceLength = dataset.get(0).get(0).getInstance().length();
	...
	private double likelihoodOfParameter(final Vector w, final DyadRankingDataset dataset) {
		...
		for (int smallN = 0; smallN < largeN; smallN++) {
			...
			int mN = dyadRankingInstance.getNumFeatures();
			double innerProduct = 1.0;
			for (int m = 0; m < mN; m++) {
				Dyad dyad = dyadRankingInstance.get(m);
				Vector zNM = this.featureTransform.transform(dyad);
				double en = Math.exp(w.dotProduct(zNM));
				double denumSum = 0;
				for (int l = m; l < mN; l++) {
					Dyad dyadL = dyadRankingInstance.get(l);
