...
public class KendallsTauDyadRankingLoss implements IDyadRankingLossFunction {
	@Override
	public double loss(final IRanking<Dyad> actual, final IRanking<Dyad> predicted) {
		...
		for (int predIndex = 0; predIndex < dyadRankingLength - 1; predIndex++) {
			Dyad predDyad = predicted.get(predIndex);
			int actualIndex = -1;
	...
	private boolean isRankingCorrectForIndex(final IRanking<Dyad> actual, final IRanking<Dyad> predicted, final int dyadRankingLength, final int actualIndex, final int i) {
		Dyad predPairedDyad = predicted.get(i);
		boolean found = false;
