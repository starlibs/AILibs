...
public class MeanSquaredLogarithmicError extends ARegressionMeasure {
	...
	public double loss(final List<? extends Double> expected, final List<? extends Double> predicted) {
		return StatisticsUtil.mean(IntStream.range(0, expected.size()).mapToObj(x -> Double.valueOf(SQUARED_LOGARITHMIC_LOSS.loss(expected.get(x), predicted.get(x)))).collect(Collectors.toList()));
	}
