package ai.libs.jaicore.ml.ranking.dyadranking.activelearning;
...
public class DyadDatasetPoolProvider implements IDyadRankingPoolProvider {
	...
	private void addDyadRankingInstance(final IDyadRankingInstance instance) {
		...
		this.dyadRankingsByInstances.put(instance.get(0).getInstance(), instance);
		this.dyadRankingsByAlternatives.put(instance.get(0).getAlternative(), instance);
	...
	private int getPositionInRankingByInstanceFeatures(final Dyad dyad) {
		if (!this.dyadRankingsByInstances.containsKey(dyad.getInstance())) {
			...
			boolean found = false;
			int curPos = 0;
			while (curPos < ranking.getNumFeatures() && !found) {
				Dyad dyadInRanking = ranking.get(curPos);
