package ai.libs.jaicore.ml.ranking.label.learner.clusterbased.modifiedisac.evalutation;
...
public class ModifiedISACEvaluator {
	...
	public static double[] evaluateModifiedISACLeaveOneOut(final ISingleLabelClassificationDataset data) throws TrainingException {
		...
		for (int i = 0; i < data.size(); i++) {
			...
			ISingleLabelClassificationDataset trainingData = (ISingleLabelClassificationDataset) data.createEmptyCopy();
			trainingData.addAll(data);

			ISingleLabelClassificationDataset testprep = (ISingleLabelClassificationDataset) data.createEmptyCopy();
			testprep.addAll(data);

			ISingleLabelClassificationDataset tester = (ISingleLabelClassificationDataset) data.createEmptyCopy();

			ISingleLabelClassificationInstance reminder = tester.get(i);
			trainingData.remove(i);

			double[] overallranking = new double[22];
			int[] totalvalue = new int[22];

			for (ISingleLabelClassificationInstance inst : trainingData) {
				int tmp = 0;
				for (int l = inst.getNumAttributes() - 1; l >= 104; l--) {
			...
			for (int p = 0; p < overallranking.length; p++) {
				overall.put(data.getListOfAttributes().get((data.size() - 1) - p).get(exampel).attribute().name(), overallranking[p]);
			}
