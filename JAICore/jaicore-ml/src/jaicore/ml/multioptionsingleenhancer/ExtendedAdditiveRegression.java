...
public class ExtendedAdditiveRegression extends MultipleClassifiersCombiner implements OptionHandler, 
  ...
  public void initializeClassifier(Instances data) throws Exception {
    ...
    m_ClassifiersArrayList = new ArrayList<Classifier>(m_Classifiers.length);
  ...
  public boolean next() throws Exception {
    ...
    if ((!m_SuitableData) || (m_ClassifiersArrayList.size() <= next_iterator) ||
            (m_Diff <= Utils.SMALL)) {
      return false;
    }

    // Build the classifier
    m_ClassifiersArrayList.add(AbstractClassifier.makeCopy(m_Classifiers[next_iterator]));
    m_ClassifiersArrayList.get(m_ClassifiersArrayList.size() - 1).buildClassifier(m_Data);

    m_Data = residualReplace(m_Data, m_ClassifiersArrayList.get(m_ClassifiersArrayList.size() - 1));
    double sum = 0;
    for (int i = 0; i < m_Data.numInstances(); i++) {
      if (getMinimizeAbsoluteError()) {
  ...
  public double classifyInstance(Instance inst) throws Exception {
    ...
    for (Classifier classifier : m_ClassifiersArrayList) {
  ...
  public double measureNumIterations() {
    return m_ClassifiersArrayList.size();
  }
  ...
  public String toString() {
    ...
    if (m_SuitableData && m_ClassifiersArrayList == null) {
    ...
    for(int i = 0; i < m_ClassifiersArrayList.size(); i++) {
    	text.append("Base classifier " 
    			+ getClassifier(i).getClass().getName()
    			+ "\n\n");
    }
    text.append("" + m_ClassifiersArrayList.size() + " models generated.\n");

    for (int i = 0; i < m_ClassifiersArrayList.size(); i++) {
      text.append("\nModel number " + i + "\n\n" +
    		  m_ClassifiersArrayList.get(i) + "\n");
    }
