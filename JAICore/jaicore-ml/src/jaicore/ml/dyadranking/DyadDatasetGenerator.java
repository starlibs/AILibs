...
public class DyadDatasetGenerator {
	...
	public static void main(String[] args) throws SQLException, TrainingException {
		...
		String resultTableName = "results_automl_metadata";
		try {
...
//			if there is no table, create one
		...
		int dyadRankingLengthTest = Integer.parseInt(args[6]);
		int numTestInstances = Integer.parseInt(args[7]);
		DyadRankingDataset trainDataset = getSparseDyadDataset(42, numTrainingInstances, dyadRankingLengthTrain,
				split.trainDatasetIds);
		DyadRankingDataset testDataset = getSparseDyadDataset(43, numTestInstances, dyadRankingLengthTest,
				split.testDatasetIds);
		try {
			PLNetDyadRanker ranker = new PLNetDyadRanker();
			ranker.train(trainDataset);
			double kendallTau = DyadRankingLossUtil.computeAverageLoss(new KendallsTauDyadRankingLoss(), testDataset,
					ranker);
