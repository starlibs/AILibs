...
public class TimeSeriesDataset implements IDataset<TimeSeriesInstance> {
    ...
    private HashMap<TimeSeriesAttributeType, INDArray> timeSeriesMatricies;

    /**
     * Constructor.
     * 
     * @param capacity        Capacity i.e. maximum number of instances the dataset
     *                        can contain.
     * @param attributesTypes Attribute type of target variable
     * @param target          Attribute type of target variable.
     */
    public TimeSeriesDataset(final int capacity, final List<IAttributeType<?>> attributesTypes,
        ...
        this.target = target;
        this.timeSeriesMatricies = new HashMap();
        ...
        for (IAttributeType<?> attributeType : attributesTypes) {
            if (attributeType instanceof TimeSeriesAttributeType) {
                ...
                timeSeriesMatricies.put(tsAttributeType, matrix);
            }
        }
    }
     ...
     * 
     * @param attributeType Attribute type to update.
     * @param matrix        The new matrix for the given attribute type.
     */
    public void updateTimeSeriesMatrix(TimeSeriesAttributeType attributeType, INDArray matrix) {
        ...
        // Update matrix.
        timeSeriesMatricies.put(attributeType, matrix);
     ...
     * 
     * @param attributeType The time series attribute type to retrieve the matrix
     *                      for.
     * @return The matrix for the given attribute type.
     */
    INDArray getMatrixForAttributeType(TimeSeriesAttributeType attributeType) {
        INDArray matrix = timeSeriesMatricies.get(attributeType);
        if (matrix == null)
    ...
    public boolean add(TimeSeriesInstance instance) {
        ...
        for (IAttributeValue<?> attributeValue : instance.getAttributeValues()) {
            if (attributeValue instanceof TimeSeriesAttributeValue) {
                ...
                INDArray attributeMatrix = timeSeriesMatricies.get(timeseriesAttributeValue.getType());
