...
def serialize_model(classifier_instance):
    ...
    arff_name = get_filename(sys.argv["arff"])
...
def serialize_prediction(prediction):
    ...
    model_path = sys.argv["model"]
    json_file = get_filename(arff_path) + "_" + get_filename(model_path) + ".json"
    ...
    print("Test ARFF: " + os.path.basename(arff_path))
