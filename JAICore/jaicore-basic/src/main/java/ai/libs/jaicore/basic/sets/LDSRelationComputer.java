		...
		case ACTIVE:
			...
			while (!this.open.isEmpty() && (next = this.open.poll()).indexOfSet < this.numSets - 1) {
				...
				for (int j = 0; j < n; j++) {
					...
					assert (System.currentTimeMillis() - innerTimePoint) < 10 : "Copying the " + (next.indexOfSet) + "-tuple " + this.currentTuple + " took " + (System.currentTimeMillis() - innerTimePoint) + "ms, which is way too much!";
