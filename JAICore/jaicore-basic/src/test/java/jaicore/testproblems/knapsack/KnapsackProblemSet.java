...
public class KnapsackProblemSet extends AAlgorithmTestProblemSet<KnapsackProblem> implements IAlgorithmTestProblemSetForSolutionIterators<KnapsackProblem, Set<String>>  {
	...
	public double getValueOfKnapsack(final KnapsackConfiguration knapsack) {
		if (knapsack == null || knapsack.getPackedObjects() == null || knapsack.getPackedObjects().size() == 0) {
			...
			for (String object : knapsack.getPackedObjects()) {
				value += this.values.get(object);
			}
			for (Set<String> bonusCombination : this.bonusPoints.keySet()) {
				...
				if (allContained) {
					value += this.bonusPoints.get(bonusCombination);
				}
			}
			return value;
		}
