...
public class SCC2018MLTest extends MultiClassClassificationExperimentRunner {
  ...
  public SCC2018MLTest(final File datasetFolder) throws IOException {
    super(datasetFolder, getClassifierNames(), getSetupNames(), timeouts, seeds, trainingPortion, numCPUs, memoryInMB, PMMulticlass.errorRate,
        new MySQLMLPlanExperimentLogger("isys-db.cs.upb.de", "mlplan", "UMJXI4WlNqbS968X", "mlplan_results_test"));
    this.logger = (MySQLMLPlanExperimentLogger) this.getLogger();
  }
    ...
    try {
      switch (algoName) {
        case "MLS-Plan": {
          ...
          // bs.setExperimentLogger(this.logger);
          // evaluator.getMeasurementEventBus().register(this.logger);
          return bs;
        }
      }
    } catch (Throwable e) {
