...
import util.search.graphgenerator.events.GraphInitializedEvent;
import util.search.graphgenerator.events.NodeReachedEvent;
...
public class ORGraphSearch<T, A, V extends Comparable<V>> {
	...
	protected Node<T, V> newNode(Node<T,V> parent, T t2) {
		assert !ext2int.containsKey(t2) : "Generating a second node object for " + t2 + " as successor of " + parent.getPoint() + " was contained as " + ext2int.get(t2).getPoint() + ", but ORGraphSearch currently only supports tree search!";
		Node<T,V> node = newNode(parent, t2, null);
