...
public class Util {
	...
	public static Collection<String> getOverwrittenDatacontainersInState(Monom state) {
		return state.stream().filter(l -> l.getPropertyName().equals("overwritten")).map(l -> l.getParameters().get(0).getName())
	...
	public static <N, A, V extends Comparable<V>> ComponentInstance getSolutionCompositionForNode(final IHASCOSearchSpaceUtilFactory<N, A, V> searchSpaceUtilFactory,
			final Collection<Component> components, Monom initState, final Node<N, ?> path) {
	...
	public static Map<Parameter, ParameterDomain> getUpdatedDomainsOfComponentParameters(Monom state, Component component, String objectIdentifierInState) {
		Map<String, String> parameterContainerMap = new HashMap<>();
		...
		for (Parameter p : component.getParameters()) {
			...
			assert assignedValue != null : "No value has been assigned to parameter " + p.getName() + " stored in container " + parameterContainerMap.get(p.getName())
					+ " in state " + state;
	...
	public static Map<Parameter, ParameterDomain> getUpdatedDomainsOfComponentParameters(Component component, Map<Parameter, String> currentValues,
			Collection<Parameter> parametersForWhichADecisionHasBeenMade) {
...
//					domains.put(param, intersection);
	...
	public static boolean isDependencyPremiseSatisfied(Dependency dependency, Map<Parameter, String> values) {
	...
	public static boolean isDependencyConditionSatisfied(Collection<Pair<Parameter, ParameterDomain>> condition, Map<Parameter, String> values) {
