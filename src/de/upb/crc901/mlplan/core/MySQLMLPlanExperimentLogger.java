...
public class MySQLMLPlanExperimentLogger extends MySQLExperimentDatabaseHandle {
	...
	private static final Logger logger = LoggerFactory.getLogger(MySQLMLPlanExperimentLogger.class);
	private int jobId;
	...
	public void addEvaluationEntry(Classifier parentSearchAlgorithm, MLServicePipeline identifiedPipeline, Double score, Object errorObject) {
		...
		try {
			...
			values.add(getErrorObjectEncoding(errorObject));
			insert("INSERT INTO `evaluations_mls` (run_id, preprocessor, classifier, errorRate, time_train, time_predict,exception) VALUES (?,?,?,?,?,?,?)",
					values);
		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
	...
	public void receivePipelineMeasurementEvent(ClassifierMeasurementEvent<Double> event) {
		...
		try {
			...
			logger.info("Received measurement event for classifier type {} with error {} and score {}", c.getClass().getName(), event.getError(),
					event.getScore());
