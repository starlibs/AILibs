...
import util.search.core.ANDORGraphSearch;
import util.search.core.AndNode;
...
public class GeneralBestFirst<T,A> extends ANDORGraphSearch<T,A,Integer> {
	...
	public GeneralBestFirst(GraphGenerator<T,A> graphGenerator, GeneralBestFirstEvaluationOrSelector<T> orAggregator, GeneralBestFirstEvaluationAggregation<T> andAggregator, NodeEvaluator<T,Integer> nodeEvaluator) {
	...
	protected Collection<Node<T,Integer>> expand(Node<T,Integer> expanded) {
		...
		/* add nodes to open */
		for (Node<T,Integer> node : newSuccessorsAndTheirScores.keySet().stream().sorted((n1,n2) -> newSuccessorsAndTheirScores.get(n1) - newSuccessorsAndTheirScores.get(n2)).collect(Collectors.toList())) {
