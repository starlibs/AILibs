...
public class ImageAutoMLTest {
	...
	public static void main(final String[] args) throws Exception {
		...
		System.out.println(stratifiedSplit.get(0).getIntermediateInstances().size() + " "
				+ stratifiedSplit.get(1).getIntermediateInstances().size());

		System.out.println("Instantiate AutoFEWekaPipeline...");
		FilterPipelineFactory factory = new FilterPipelineFactory(
				parsedDataSet.getIntermediateInstances().get(0).shape());
		...
		ComponentInstance filter = new ComponentInstance(
				componentMap.get("autofe.algorithm.hasco.filter.image.CatalanoWrapperFilter"), filterParameterValues,
				new HashMap<>());

		ComponentInstance extractor = new ComponentInstance(
				componentMap.get("autofe.algorithm.hasco.filter.image.LocalBinaryPatternFilter"), new HashMap<>(),
				new HashMap<>());
		...
		ComponentInstance abstractPipe = new ComponentInstance(componentMap.get("AbstractPipe"), new HashMap<>(),
				abstractPipeSubComponents);
		...
		ComponentInstance filterPipeline = new ComponentInstance(componentMap.get("pipeline"), new HashMap<>(),
				filterPipelineSubComponents);
