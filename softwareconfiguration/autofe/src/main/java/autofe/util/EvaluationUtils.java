...
public final class EvaluationUtils {
	...
	/**
	 * Penalizes high count of attributes (which leads to high run times of attribute selectors and classifier training).
	 *
	 * @param instances
	 * @return
	 */
	...
	public static double evaluateMLPlan(final int timeout, final Instances training, final Instances test, final int seed, final Logger logger, final boolean enableVisualization, final int numCores) throws Exception {
		...
		builder.withTimeOut(new TimeOut(timeout, TimeUnit.MILLISECONDS)).withNumCpus(numCores);
