...
public class AutoFEMLPreferredNodeEvaluator implements INodeEvaluator<TFDNode, Double> {
    ...
    public AutoFEMLPreferredNodeEvaluator(final Collection<Component> components,
        ...
        classifiers = FileUtils.readLines(new File("model/weka/precedenceList.txt"), Charset.defaultCharset());
    }

    public AutoFEWekaPipeline getPipelineFromNode(final Node<TFDNode, ?> node) throws ComponentInstantiationFailedException {
        return factory.getComponentInstantiation(getComponentInstanceFromNode(node));
    }
    ...
    public Double f(final Node<TFDNode, ?> node) throws NodeEvaluationException {
        ...
        ComponentInstance ci = Util.getSolutionCompositionFromState(components, node.getPoint().getState(), true);

        AutoFEWekaPipeline pipe;
        try {
            pipe = getPipelineFromComponentInstance(ci);
        ...
        if (toDoHasAlgorithmSelection) {
            if (pipe != null) {
                ...
                if (pipe.getFilterPipeline() != null && pipe.getFilterPipeline().getFilters() != null) {
                    return calculateScoreForFilterPipeline(pipe);
                }
                ...
                if (ci != null && ci.getSatisfactionOfRequiredInterfaces().containsKey("mlPipeline")) {
                    ...
                    int index = classifiers.indexOf(classifierName);
                    if (index > 0) {
                        score += index;
                    } else {
                        score += classifiers.size() + 1;
    ...
    private double calculateScoreForFilterPipeline(final AutoFEWekaPipeline pipe) {
        if (pipe.getFilterPipeline().getFilters().getItems().size() > maxPipelineSize) {
        ...
        double indexOfClassifierName = classifiers.indexOf(classifierName);
        if (indexOfClassifierName < 0) {
            indexOfClassifierName = classifiers.size() + 1d;
        }
        ...
        score += numFilters * (classifiers.size() + 1d * 2d);
        score /= 100000d;
