...
SoftwareConfigurationAlgorithm<TwoPhaseSoftwareConfigurationProblem, HASCOSolutionCandidate<Double>, Double> {
	...
	public AlgorithmEvent nextWithException() throws InterruptedException, AlgorithmTimeoutedException, AlgorithmException, AlgorithmExecutionCanceledException {
		...
		case created: {
			...
			this.logger.info(
					...
					this.getNumCPUs(), this.getTimeout(), this.getConfig().timeoutForNodeEvaluation(),
					this.getConfig().timeoutForCandidateEvaluation(), this.getConfig().numberOfRandomCompletions(),
					...
					try {
						while (!Thread.currentThread().isInterrupted()) {
							...
							if (timeRemaining < 2000 || TwoPhaseHASCO.this.shouldSearchTerminate(timeRemaining)) {
								TwoPhaseHASCO.this.logger.info("Canceling HASCO (first phase). {}ms remaining.",
										timeRemaining);
	...
	private synchronized List<HASCOSolutionCandidate<Double>> getSelectionForPhase2(final int remainingTime) {
		...
		Collection<HASCOSolutionCandidate<Double>> potentialCandidates = new ArrayList<>(this.phase1ResultQueue)
	...
	public int getExpectedTotalRemainingRuntimeForAGivenPool(final Collection<HASCOSolutionCandidate<Double>> solutions,
			final boolean assumeCurrentlyBestCandidateToBeSelected) {
	...
	public int getPostprocessingTimeOfCurrentlyBest() {
		return (int) Math.round(this.getBestSeenSolution().getTimeToEvaluateCandidate()
				* this.getConfig().expectedBlowupInSelection() * this.getConfig().expectedBlowupInPostprocessing());
	}
	...
	public int getExpectedRuntimeForPhase2ForAGivenPool(final Collection<HASCOSolutionCandidate<Double>> solutions) {
		...
		int estimateEvaluationTimeForSelectionPhase = (int) (inSearchMCEvalTime
				* this.getConfig().expectedBlowupInSelection());
		...
		this.logger.debug("Expected runtime is {} = {} * {} / {} for a pool of size {}", runtime, inSearchMCEvalTime,
				this.getConfig().expectedBlowupInSelection(), usableCPUs, solutions.size());
		return runtime;
	}
	...
	protected HASCOSolutionCandidate<Double> selectModel() {
		...
		if (this.getTimeout().seconds() > 0) {
			int remainingTime = (int) (this.getTimeout().milliseconds()
			...
			pool.submit(new Runnable() {
				...
				public void run() {
					...
					try {
						...
						TwoPhaseHASCO.this.logger.info(
								"Selection eval of {} got interrupted after {}ms. Defined timeout was: {}ms",
	...
	private synchronized int getCandidateThatWouldCurrentlyBeSelectedWithinPhase2(
		...
		for (int i = 0; i < ensembleToSelectFrom.size(); i++) {
			...
			// if (logComputations) {
			// logger.info("Ignoring candidate {} because no results were obtained in
			...
			// if (logComputations) {
			// logger.info("Score of candidate {} is {} based on {} (avg) and {} (.75-pct)
			// with {} samples", candidate, score, avgError, quartileScore,
	...
	public TwoPhaseHASCOReport getReort() {
		return new TwoPhaseHASCOReport(this.phase1ResultQueue.size(), this.secondsSpentInPhase1,
				this.selectedHASCOSolution);
	}
