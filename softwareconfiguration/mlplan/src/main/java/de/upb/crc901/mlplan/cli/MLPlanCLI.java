...
public class MLPlanCLI {
	...
	private static Options generateOptions() {
		// MLPLan options
		final Option train = Option.builder("t")
				.required()
				.hasArg()
				.longOpt(trainOption)
				.desc("location of the .arff training data file")
				.build();
		final Option test = Option.builder("T")
				.required(false)
				.longOpt(testOption)
				.hasArg()
				.desc("location of the .arff test data file")
				.build();
		final Option totalTimeout = Option.builder("tt")
				.longOpt(totalTimeoutOption)
				.required(false)
				.hasArg()
				.desc("timeout for the complete run of mlplan in seconds")
				.build();
		final Option nodeEvaluationTimeout = Option.builder("tne")
				.longOpt(nodeEvaluationTimeoutOption)
				.required(false)
				.hasArg()
				.desc("timeout for the evaluation of a single node in seconds")
				.build();
		final Option solutionEvaluation = Option.builder("tse")
				.longOpt(solutionEvaluationTimeoutOption)
				.required(false)
				.hasArg()
		...
		final Option searchSpaceConfiguration = Option.builder("sc")
				...
				.required(false)
				.hasArg()
				.desc("search space configuration file, or alternatively: autoweka, meka, autosklearn, tpot, or tinytest")
				.build();
		final Option evaluationMeasure = Option.builder("em")
				.longOpt(evaluationMeasureOption)
				.required(false)
				.hasArg()
				.desc("measure for assessing solution quality, allowed values: \nsinglelabel: \nERRORRATE, MEAN_SQUARED_ERROR, ROOT_MEAN_SQUARED_ERROR \nmultilabel: \nAUTO_MEKA_GGP_FITNESS, AUTO_MEKA_GGP_FITNESS_LOSS, EXACT_MATCH_ACCURARY, EXACT_MATCH_LOSS, F1_MACRO_AVG_D, F1_MACRO_AVG_D_LOSS, F1_MACRO_AVG_L, F1_MACRO_AVG_L_LOSS,  HAMMING_ACCURACY, HAMMING_LOSS, JACCARD_LOSS, JACCARD_SCORE, RANK_LOSS, RANK_SCORE")
				.build();
		final Option numCPUS = Option.builder("ncpus")
				.longOpt(numCPUsOption)
				.required(false)
				.hasArg()
				.desc("number of used CPUs, default: " + MLPlanCLI.numCPUS)
				.build();
		final Option randomSeed = Option.builder("rs")
				.longOpt(randomSeedOption)
				.required(false)
				.hasArg()
				.desc("randomization seed, default: " + MLPlanCLI.randomSeed)
				.build();
		final Option multiLabel = Option.builder("ml")
				.longOpt(multiLabelOption)
				.required(false)
				.hasArg(false)
				.desc("enable for multilabel settings")
				.build();
		
		// Communication options
		final Option modelFile = Option.builder("mf")
				.longOpt(modelFileOption)
				.required(false)
				.hasArg()
				.desc("serialize model to the given output file, \"off\" if no model file shall be written; turn off for search spaces that contain non-serializable models")
				.build();
		final Option resultsFile = Option.builder("rf")
				.longOpt(resultsFileOption)
				.required(false)
				.hasArg()
				.desc("serialize model to the given output file, \"off\" if no results file shall be written")
				.build();
		final Option visualize = Option.builder("v")
				.longOpt(visualizeOption)
				.required(false)
				.hasArg(false)
				.desc("enable visualization")
				.build();
		final Option printModel = Option.builder("p")
				.longOpt(printModelOption)
				.required(false)
				.hasArg(false)
				.desc("whether a visual representation of the final model shall be added to the model file")
				.build();
	...
	private static void runMLPlan(CommandLine commandLine) throws Exception {
		...
		if (commandLine.hasOption(evaluationMeasureOption)) {
			if (commandLine.hasOption(multiLabelOption)) {
				...
				case "AUTO_MEKA_GGP_FITNESS": builder.withMultiLabelClassificationMeasure(EMultilabelPerformanceMeasure.AUTO_MEKA_GGP_FITNESS); break;
				case "AUTO_MEKA_GGP_FITNESS_LOSS" : builder.withMultiLabelClassificationMeasure(EMultilabelPerformanceMeasure.AUTO_MEKA_GGP_FITNESS_LOSS); break;
				case "EXACT_MATCH_ACCURARY"  : builder.withMultiLabelClassificationMeasure(EMultilabelPerformanceMeasure.EXACT_MATCH_ACCURARY); break; 
				case "EXACT_MATCH_LOSS" : builder.withMultiLabelClassificationMeasure(EMultilabelPerformanceMeasure.EXACT_MATCH_LOSS); break; 
				case "F1_MACRO_AVG_D" : builder.withMultiLabelClassificationMeasure(EMultilabelPerformanceMeasure.F1_MACRO_AVG_D); break;  
				case "F1_MACRO_AVG_D_LOSS" : builder.withMultiLabelClassificationMeasure(EMultilabelPerformanceMeasure.F1_MACRO_AVG_D_LOSS); break; 
				case "F1_MACRO_AVG_L" : builder.withMultiLabelClassificationMeasure(EMultilabelPerformanceMeasure.F1_MACRO_AVG_L); break; 
				case "F1_MACRO_AVG_L_LOSS" : builder.withMultiLabelClassificationMeasure(EMultilabelPerformanceMeasure.F1_MACRO_AVG_L_LOSS); break;  
				case "HAMMING_ACCURACY" : builder.withMultiLabelClassificationMeasure(EMultilabelPerformanceMeasure.HAMMING_ACCURACY); break; 
				case "HAMMING_LOSS" : builder.withMultiLabelClassificationMeasure(EMultilabelPerformanceMeasure.HAMMING_LOSS); break; 
				case "JACCARD_LOSS" : builder.withMultiLabelClassificationMeasure(EMultilabelPerformanceMeasure.JACCARD_LOSS); break; 
				case "JACCARD_SCORE" : builder.withMultiLabelClassificationMeasure(EMultilabelPerformanceMeasure.JACCARD_SCORE); break;
				case "RANK_LOSS" : builder.withMultiLabelClassificationMeasure(EMultilabelPerformanceMeasure.RANK_LOSS); break; 
				case "RANK_SCORE" : builder.withMultiLabelClassificationMeasure(EMultilabelPerformanceMeasure.RANK_SCORE); break;
				default : throw new IllegalArgumentException("Invalid multilabel measure " + commandLine.getOptionValue(evaluationMeasureOption));
				}
			} else {
				switch(commandLine.getOptionValue(evaluationMeasureOption)) {
				case "ERRORRATE" : builder.withSingleLabelClassificationMeasure(EMultiClassPerformanceMeasure.ERRORRATE); break; 
				case "MEAN_SQUARED_ERROR" : builder.withSingleLabelClassificationMeasure(EMultiClassPerformanceMeasure.MEAN_SQUARED_ERROR); break; 
				case "ROOT_MEAN_SQUARED_ERROR" : builder.withSingleLabelClassificationMeasure(EMultiClassPerformanceMeasure.ROOT_MEAN_SQUARED_ERROR);break;
				default : throw new IllegalArgumentException("Invalid singlelabel measure " + commandLine.getOptionValue(evaluationMeasureOption));
				}
			}
		}
		if (commandLine.hasOption(searchSpaceConfigurationOption)) {
			switch (commandLine.getOptionValue(searchSpaceConfigurationOption)) {
			case "autoweka" : builder.withAutoWEKAConfiguration(); break;
			case "meka" : builder.withMekaDefaultConfiguration(); break;
			case "tpot" : builder.withTpotConfig(); break;
			case "autosklearn" : builder.withAutoSKLearnConfig(); break;
			case "tinytest" : builder.withTinyTestConfiguration();break;
			default : builder.withSearchSpaceConfigFile(new File(commandLine.getOptionValue(searchSpaceConfigurationOption)));
			}
		} else {
		...
		if (commandLine.hasOption(testOption)) {
			...
			if (commandLine.hasOption(multiLabelOption)) {
				...
				case "AUTO_MEKA_GGP_FITNESS": error = (Metrics.P_ExactMatch(result.allTrueValues(), result.allPredictions(0.5)) + (1 - Metrics.L_Hamming(result.allTrueValues(), result.allPredictions(0.5)))
						+ Metrics.P_FmacroAvgL(result.allTrueValues(), result.allPredictions(0.5)) + (1 - Metrics.L_RankLoss(result.allTrueValues(), result.allPredictions()))) / 4.0; break;
				case "AUTO_MEKA_GGP_FITNESS_LOSS" : error = 1 - (Metrics.P_ExactMatch(result.allTrueValues(), result.allPredictions(0.5)) + (1 - Metrics.L_Hamming(result.allTrueValues(), result.allPredictions(0.5)))
						+ Metrics.P_FmacroAvgL(result.allTrueValues(), result.allPredictions(0.5)) + (1 - Metrics.L_RankLoss(result.allTrueValues(), result.allPredictions()))) / 4.0; break;
				case "EXACT_MATCH_ACCURARY"  : error = Metrics.P_ExactMatch(result.allTrueValues(), result.allPredictions(0.5)); break; 
				case "EXACT_MATCH_LOSS" : error = 1 - Metrics.P_ExactMatch(result.allTrueValues(), result.allPredictions(0.5)); break; 
				case "F1_MACRO_AVG_D" : error =  Metrics.P_FmacroAvgD(result.allTrueValues(), result.allPredictions(0.5)); break;
				case "F1_MACRO_AVG_D_LOSS" : error = 1 - Metrics.P_FmacroAvgD(result.allTrueValues(), result.allPredictions(0.5)); break; 
				case "F1_MACRO_AVG_L" :  error = Metrics.P_FmacroAvgL(result.allTrueValues(), result.allPredictions(0.5)); break; 
				case "F1_MACRO_AVG_L_LOSS" : error = 1 -  Metrics.P_FmacroAvgL(result.allTrueValues(), result.allPredictions(0.5)); break;  
				case "HAMMING_ACCURACY" : error = Metrics.P_Hamming(result.allTrueValues(), result.allPredictions(0.5)); break; 
				case "HAMMING_LOSS" : error = Metrics.L_Hamming(result.allTrueValues(), result.allPredictions(0.5)); break; 
				case "JACCARD_LOSS" : error = Metrics.L_JaccardDist(result.allTrueValues(), result.allPredictions(0.5)); break; 
				case "JACCARD_SCORE" : error = Metrics.P_JaccardIndex(result.allTrueValues(), result.allPredictions(0.5)); break;
				case "RANK_LOSS" : error = Metrics.L_RankLoss(result.allTrueValues(), result.allPredictions()); break; 
				case "RANK_SCORE" : error = 1 - Metrics.L_RankLoss(result.allTrueValues(), result.allPredictions()); break;
				default : throw new IllegalArgumentException("Invalid multilabel measure " + commandLine.getOptionValue(evaluationMeasureOption));
				}
				...
				case "ERRORRATE" : error = eval.errorRate(); break;
				case "MEAN_SQUARED_ERROR" : error = Math.pow(eval.rootMeanSquaredError(), 2); break;
				case "ROOT_MEAN_SQUARED_ERROR" : error = eval.rootMeanSquaredError(); break;
				default : throw new IllegalArgumentException("Invalid singlelabel measure " + commandLine.getOptionValue(evaluationMeasureOption));
				}
	...
	private static void writeSingleLabelEvaluationFile(Evaluation eval, double internalError, CommandLine commandLine, Classifier bestModel) throws Exception {
		StringBuilder builder = new StringBuilder();
		builder.append("Internally believed error: ");
		builder.append(internalError);
		builder.append(System.lineSeparator());
