...
public class SelectionPhasePipelineEvaluator
		implements IObjectEvaluator<ComponentInstance, Double>, ILoggingCustomizable {
	...
	public SelectionPhasePipelineEvaluator(ClassifierFactory classifierFactory,
			AbstractEvaluatorMeasureBridge<Double, Double> evaluationMeasurementBridge, int numMCIterations,
			Instances dataShownToSearch, double trainFoldSize, int seed) {
		super();
	...
	@Override
	public Double evaluate(ComponentInstance c)
	...
	private Double evaluate(ComponentInstance c, AbstractEvaluatorMeasureBridge<Double, Double> bridge)
			throws ObjectEvaluationFailedException, InterruptedException {
		MonteCarloCrossValidationEvaluator mccv = new MonteCarloCrossValidationEvaluator(bridge, numMCIterations,
				dataShownToSelectionPhase, trainFoldSize, seed);
		...
		logger.info("Select {} as .75-percentile where {} would have been the mean. Samples size of MCCV was {}",
