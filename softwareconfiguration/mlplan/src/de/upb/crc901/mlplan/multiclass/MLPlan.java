...
public class MLPlan extends AbstractClassifier
	...
	public void buildClassifier(final Instances data) throws Exception {
		...
		MonteCarloCrossValidationEvaluator solutionEvaluatorInPhase2 = new MonteCarloCrossValidationEvaluator(
				new MulticlassEvaluator(new Random(3)), 3, data, .7f);
			...
			for (int j = 0; j < this.numberOfMCIterationsPerSolutionInSelectionPhase; j++) {
				pool.submit(new Runnable() {
					...
					public void run() {
						int taskId = -1;
						try {
							int indexOfCurrentlyChosenModel = MLPlan.this
							...
							int estimatedOverallTrainingTimeForChosenModel = (int) Math
									.round(trainingTimeForChosenModelInsideSearch
											/ (1 - MLPlan.this.portionOfDataForPhase2) / .7); // we
																								// assume a
																								// linear
																								// growth
