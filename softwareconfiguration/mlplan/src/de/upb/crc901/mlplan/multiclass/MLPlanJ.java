...
public class MLPlanJ extends AbstractClassifier
	...
	private final HASCOForWekaMLJ hasco;
	...
	public void buildClassifier(final Instances data) throws Exception {
		...
		logger.info("Starting ML-Plan with timeout {}s, and a portion of {} for the second phase.", this.timeoutInS,
				this.portionOfDataForPhase2);
			...
			public void run() {
				try {
					while (!Thread.interrupted()) {
						Thread.sleep(100);
						int timeElapsed = (int) (System.currentTimeMillis() - MLPlanJ.this.timeOfStart);
						int timeRemaining = MLPlanJ.this.timeoutInS * 1000 - timeElapsed;
						if (MLPlanJ.this.shouldSearchTerminate(timeRemaining)) {
							MLPlanJ.this.hasco.cancel();
							return;
						}
					}
	...
	protected boolean shouldSearchTerminate(final long timeRemaining) {
		Collection<HASCOForWekaMLSolutionJ> currentSelection = this.getSelectionForPhase2();
		int estimateForPhase2 = this.isSelectionActivated()
				? this.getExpectedRuntimeForPhase2ForAGivenPool(currentSelection)
				: 0;
		HASCOForWekaMLSolutionJ internallyOptimalSolution = this.hasco.getCurrentlyBestSolution();
		...
		logger.info("{}ms remaining in total, and we estimate {}ms for phase 2. Terminate phase 1: {}", timeRemaining,
				estimateForPhase2, terminatePhase1);
		return terminatePhase1;
	}

	private synchronized List<HASCOForWekaMLSolutionJ> getSelectionForPhase2() {
		return this.getSelectionForPhase2(Integer.MAX_VALUE);
	}

	private synchronized List<HASCOForWekaMLSolutionJ> getSelectionForPhase2(final int remainingTime) {
		...
		if (!this.isSelectionActivated()) {
			List<HASCOForWekaMLSolutionJ> best = new ArrayList<>();
		...
		Collection<HASCOForWekaMLSolutionJ> potentialCandidates = new ArrayList<>(this.hasco.getFoundClassifiers())
		...
		List<HASCOForWekaMLSolutionJ> selectionCandidates = potentialCandidates.stream().limit(bestK)
				.collect(Collectors.toList());
		List<HASCOForWekaMLSolutionJ> remainingCandidates = new ArrayList<>(
		...
		List<HASCOForWekaMLSolutionJ> actuallySelectedSolutions = new ArrayList<>();
		int expectedRuntime;
		for (HASCOForWekaMLSolutionJ pl : selectionCandidates) {
	...
	private int getInSearchEvaluationTimeOfSolutionSet(final Collection<HASCOForWekaMLSolutionJ> solutions) {
		return this.hasco.getFoundClassifiers().stream().map(x -> x.getTimeToComputeScore()).reduce(0, (a, b) -> a + b)
				.intValue();
	}

	public int getExpectedRuntimeForPhase2ForAGivenPool(final Collection<HASCOForWekaMLSolutionJ> solutions) {
		...
		logger.debug("Expected runtime is {} = {} * {} * {} * {} / {}", runtime, conservativenessFactor,
	...
	protected Classifier selectModel(final ClassifierEvaluator evaluator) {
		...
		Queue<HASCOForWekaMLSolutionJ> solutions = this.hasco.getFoundClassifiers();
		HASCOForWekaMLSolutionJ bestSolution = solutions.peek();
		...
		logger.info("Starting with phase 2: Selection of final model among the {} solutions that were identified.",
				solutions.size());
		long startOfPhase2 = System.currentTimeMillis();
		List<HASCOForWekaMLSolutionJ> ensembleToSelectFrom;
		...
		HASCOForWekaMLSolutionJ selectedModel = bestSolution; // backup solution
		...
		for (int i = 0; i < ensembleToSelectFrom.size(); i++) {
			HASCOForWekaMLSolutionJ c = ensembleToSelectFrom.get(i);
			...
			for (int j = 0; j < this.numberOfMCIterationsPerSolutionInSelectionPhase; j++) {
				pool.submit(new Runnable() {
					...
					public void run() {
						int taskId = -1;
						try {
							int indexOfCurrentlyChosenModel = MLPlanJ.this
									.getClassifierThatWouldCurrentlyBeSelectedWithinPhase2(ensembleToSelectFrom, stats,
											false);
							HASCOForWekaMLSolutionJ currentlyChosenSolution = ensembleToSelectFrom
							...
							int estimatedOverallTrainingTimeForChosenModel = (int) Math
									.round(trainingTimeForChosenModelInsideSearch
											/ (1 - MLPlanJ.this.portionOfDataForPhase2) / .7); // we
																								// assume a
																								// linear
																								// growth
							int expectedTrainingTimeOfThisModel = (int) Math.round(c.getTimeToComputeScore() / .7);
							if (MLPlanJ.this.timeoutPerNodeFComputation > 0) {
								taskId = ts.interruptMeAfterMS(MLPlanJ.this.timeoutPerNodeFComputation);
							}
							if (MLPlanJ.this.timeoutInS > 0) {
		...
		try {
			...
			logger.info("Finished phase 2 within {}ms net. Total runtime was {}ms. ", endOfPhase2 - startOfPhase2,
			...
			if (ensembleToSelectFrom.isEmpty()) {
				...
				logger.info("Selected a model. The model is: {}. Its internal error was {}. Validation error was {}",
	...
	private synchronized int getClassifierThatWouldCurrentlyBeSelectedWithinPhase2(
			final List<HASCOForWekaMLSolutionJ> ensembleToSelectFrom, final List<DescriptiveStatistics> stats,
			final boolean logComputations) {
		int selectedModel = 0;
		double best = Double.MAX_VALUE;
		for (int i = 0; i < ensembleToSelectFrom.size(); i++) {
			HASCOForWekaMLSolutionJ candidate = ensembleToSelectFrom.get(i);
			DescriptiveStatistics statsOfCandidate = stats.get(i);
			if (statsOfCandidate.getN() == 0) {
				if (logComputations) {
					logger.info("Ignoring candidate {} because no results were obtained in selection phase.",
							candidate);
				}
				continue;
			}
			...
			if (logComputations) {
				logger.info("Score of candidate {} is {} based on {} (avg) and {} (.75-pct) with {} samples", candidate,
						score, avgError, quartileScore, statsOfCandidate.getN());
			}
			if (score < best) {
				best = score;
				selectedModel = i;
