...
public class DyadDatasetGenerator {
	...
	private static Pair<Dyad, Double> getDyadAndScoreWithId(int id, SQLAdapter adapter) throws SQLException {
		ResultSet res = adapter.getResultsOfQuery("SELECT " + xMetric + ", score FROM `dyad_datasets` WHERE id=" + id);
		if (res.wasNull())
			throw new IllegalArgumentException("No entry with id " + id);

		res.first();

		ResultSet res_y = adapter.getResultsOfQuery("SELECT y FROM `dyad_datasets_support_2000` WHERE id=" + id);
		res_y.first();
	...
	public static void main(String... args) throws SQLException, TrainingException {
		...
		DyadRankingDataset trainDataset = getSparseDyadDataset(42, 400, 10, split.trainDatasetIds);
		DyadRankingDataset testDataset = getSparseDyadDataset(43, 200, 10, split.testDatasetIds);
		ADyadRanker ranker = new FeatureTransformPLDyadRanker();
