...
import ai.libs.hasco.pcsbasedoptimization.PCSBasedOptimizationEventListener;
import ai.libs.hasco.pcsbasedoptimization.PCSBasedOptimizerInput;
import ai.libs.hasco.pcsbasedoptimization.SMACOptimizer;
import ai.libs.hasco.pcsbasedoptimization.ScenarioFileUtil;
...
public class PCSBasedOptimizerTest {
	...
	public void SMACBuilderTest() {
		SMACOptimizer smacOptimizer = SMACOptimizer.SMACOptimizerBuilder(input, evaluator).executionPath("testrsc")
				.algoRunsTimelimit(99).runCountLimit(11).alwaysRaceDefault(0).costForCrash(10.0).cutoff(0.0)
				.deterministic(1).memoryLimit(256).runCountLimit(10).wallClockLimit(10.0).build();
	...
	public void HyperBandBuilderTest() {
		HyperBandOptimizer optimizer = HyperBandOptimizer.HyperBandOptimizerBuilder(input, evaluator)
				.executionPath("testsrc").minBudget(10.0).maxBudget(100.0).nIterations(10).build();
	...
	public void BOHBBuilderTest() {
		BOHBOptimizer optimizer = BOHBOptimizer.BOHBOptimizerBuilder(input, evaluator).executionPath("testrsc")
				.minBudget(10.0).maxBudget(100.0).nIterations(10).build();
		String command = optimizer.setOptions();
	...
	public void EventBusTest() throws Exception {
		...
		evaluator.registerEventListener(listener);
		HyperBandOptimizer optimizer = HyperBandOptimizer.HyperBandOptimizerBuilder(input, evaluator)
				.executionPath("PCSBasedOptimizerScripts/HyperBandOptimizer").maxBudget(230.0).minBudget(9.0)
				.nIterations(4).build();
		optimizer.optimize("weka.classifiers.functions.Logistic");
